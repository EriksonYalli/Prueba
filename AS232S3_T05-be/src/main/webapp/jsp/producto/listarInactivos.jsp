...
<div class="container-fluid" id="container">
    <!-- Título de la página -->
    <h1 class="text-center my-4">Listado de Productos Inactivos</h1>
    ...
    <table id="productTable" class="table table-bordered table-striped text-center">
        ...
        <tr>
            ...
            <td>
                <a href="javascript:void(0);" onclick="confirmarRestaurar(event, <%= product.getProductoID() %>)" class="btn btn-info btn-sm">
...
<script>
    // Función para mostrar alerta de confirmación antes de restaurar
    function confirmarRestaurar(event, id) {
        event.preventDefault(); // Previene que el enlace se ejecute directamente

        Swal.fire({
            title: '¿Estás seguro?',
            text: 'Este producto será restaurado a estado activo.',
            icon: 'question',
            ...
            reverseButtons: true          // Botones en orden inverso
        }).then((result) => {
            if (result.isConfirmed) {
                // Redirige a la acción de restaurar si el usuario confirma
                window.location.href = 'producto?action=restaurar&id=' + id;
            }
        });
    }
</script>
