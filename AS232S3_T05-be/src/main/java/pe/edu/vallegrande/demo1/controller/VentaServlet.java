...
public class VentaServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        } else if ("detalleVenta".equals(action)) {
            int id = Integer.parseInt(request.getParameter("id"));
            System.out.println(id);
            HistorialSERVICE historial = new HistorialSERVICE();
            List<VentaDTO> venta = historial.ListarVenta(id);
            List<DetalleVentaDTO> detalle = historial.VerDetalleVenta(id);
            request.setAttribute("detalle", detalle);
            request.setAttribute("venta", venta);
            request.setAttribute("id", id);
            RequestDispatcher view = request.getRequestDispatcher("index.jsp?page=detalleVenta");
